...
variable "acl" {
  ...
  description = "The canned ACL to apply. See comment above for the list of possible ACLs. If not `null` bucket_ownership cannot be BucketOwnerEnforced"
  type        = string
  default     = null
}
...
variable "replica_bucket_key_enabled" {
  # See https://docs.aws.amazon.com/AmazonS3/latest/dev/bucket-key.html
  description = "Optional whether or not to use Amazon S3 Bucket Keys for SSE-KMS for the replica bucket."
  type        = bool
  default     = false
}
